...
import { feedPostUpsertSchema, contentListQuerySchema } from "@/lib/validation";
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error("[GET /api/admin/content-manager/feed]", error);
...
export async function POST(req: NextRequest) {
  ...
  const parsed = feedPostUpsertSchema.safeParse(body);
  if (!parsed.success) {
  ...
  if (error) {
    console.error("[POST /api/admin/content-manager/feed]", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  ...
  await supabase.from("admin_activity_log").insert({
    ...
    entity_id: data.id,
    details: `Created feed post: ${data.title ?? data.id}`,
  });
